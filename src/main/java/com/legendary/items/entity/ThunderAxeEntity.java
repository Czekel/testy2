...
/**
 * Rzucany Boski Topor. Po trafieniu (moba lub bloku):
 *  - uderza piorun w miejsce trafienia,
 *  - odpycha wszystkie zywe istoty w promieniu wokol GRACZA, ktory rzucil toporem.
 */
public class ThunderAxeEntity extends ThrownItemEntity {
	...
	public ThunderAxeEntity(World world, LivingEntity owner) {
		super(ModEntities.THUNDER_AXE_ENTITY, owner, world);
	}

	public ThunderAxeEntity(World world, double x, double y, double z) {
		super(ModEntities.THUNDER_AXE_ENTITY, x, y, z, world);
	}
	...
	protected void onEntityHit(EntityHitResult hitResult) {
		...
		if (!getWorld().isClient) {
			...
			if (hitEntity instanceof LivingEntity living) {
				...
				living.damage((ServerWorld) getWorld(), source, HIT_DAMAGE);
			}
	...
	private void strikeLightningAndKnockback(Vec3d hitPos) {
		...
		if (world instanceof ServerWorld serverWorld) {
			...
			// odepchniecie wszystkiego wokol gracza (wlasciciela topora)
			Entity owner = getOwner();
			if (owner != null) {
